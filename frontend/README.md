...
│   ├── routes/  # простая маршрутизация<br>
│   ├── styles/  # Глобальные стили<br>
│   └── ui/   # Базовые UI-компоненты уровня приложения (Layout)<br> 
...
- Проект настроен, все первичные необходимые зависимости установлены, создана структура проекта по принципам FSD, алиасы настроены.
- Созданы легкие провайдеры (QueryProvider, ThemeProvider, RouterProvider, AuthProvider), которые выполняют ровно одну задачу:<br>
...
- Сделан базовый API слой (создаёт готовый к использованию клиент $api, который Подставляет базовый URL; Автоматически добавляет токен в заголовки (если
он есть); При получении ошибки авторизации (401) очищает токен и перенаправляет на логин.).
